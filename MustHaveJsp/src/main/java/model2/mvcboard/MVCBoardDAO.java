...
public class MVCBoardDAO extends DBConnPool {
	...
	public boolean confirmPassword(String pass, String idx) {
		...
		try {
			...
			rs = psmt.executeQuery();
			rs.next(); // ResultSet에 결과값을 담음
			if (rs.getInt(1) == 0) { // int형으로 1번의 값을 가져왔을 때, 일치하는 값이 없다면
				isCorr = false;
			}
		} catch (Exception e) {
			isCorr = false;
	...
	public int updatePost(MVCBoardDTO dto) { // 게시글 폼의 내용을 db에 저장(파일 업로드)
		int result = 0;
		try {
			//쿼리문
